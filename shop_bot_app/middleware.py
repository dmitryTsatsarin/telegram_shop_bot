...
# this code based on django_logutils middleware 0.4.2
...
def create_log_message(log_dict, use_sql_info=False):
    ...
    """
    log_msg = (
        "{remote_address} {method} {url} {status} "
        "{request_data} ({request_time:.2f} seconds)"
    )
    if use_sql_info:
        sql_time = sum(
...
class TsdLoggingMiddleware(MiddlewareMixin, object):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        super(TsdLoggingMiddleware, self).__init__(*args, **kwargs)
